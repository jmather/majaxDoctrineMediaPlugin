...
class getid3_lib
{
		...
		if (GETID3_OS_ISWINDOWS) {
			...
			foreach ($RequiredFiles as $required_file) {
				if (!is_readable(GETID3_HELPERAPPSDIR.$required_file)) {
					die(implode(' and ', $RequiredFiles).' are required in '.GETID3_HELPERAPPSDIR.' for getid3_lib::md5_file() to public static function under Windows in PHP < v4.2.0');
				}
			}
	...
	public static function sha1_file($file) {
		...
		if (GETID3_OS_ISWINDOWS) {
			...
			foreach ($RequiredFiles as $required_file) {
				if (!is_readable(GETID3_HELPERAPPSDIR.$required_file)) {
					die(implode(' and ', $RequiredFiles).' are required in '.GETID3_HELPERAPPSDIR.' for getid3_lib::sha1_file() to public static function under Windows in PHP < v4.3.0');
				}
			}
